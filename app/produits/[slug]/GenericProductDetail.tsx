...
function DescriptionAccordion({
  ...
  return (
    <div className={className}>
      ...
      <div
        className={`relative overflow-hidden transition-[max-height] duration-300 ease-out ${expanded ? "max-h-[2000px]" : "max-h-20"}`}
      >
        <ProductDescription
          product={product}
          className="text-sm leading-relaxed text-[#5c534a]"
        />
        {!expanded && (
          <div
            className="absolute inset-x-0 bottom-0 h-10 bg-linear-to-t from-cream to-transparent"
            aria-hidden="true"
          />
        )}
      </div>
  ...
  return (
    <div>
      <div className="hidden md:block">
        <div
          ref={heroDesktopRef}
          className="mx-auto max-w-[1320px] px-4 py-10 sm:px-6 sm:py-14"
        >
          ...
          <div className="grid gap-10 md:grid-cols-2 md:items-start md:gap-14">
            <div data-reveal className="flex gap-4">
              ...
              <div
                ...
                style={{
                  aspectRatio: Math.min(1.4, Math.max(0.75, imageRatio ?? 1)),
                }}
      ...
      <div className="md:hidden">
        ...
        <div ref={heroMobileRef} className="px-4 pt-5 pb-4">
          ...
          <div
            data-reveal
            className="text-xs tracking-[0.2em] text-brown uppercase"
          >
            Anissa Cosmetics
          </div>
          <h1
            data-reveal
            className="mt-2 font-serif text-[28px] leading-tight font-semibold text-ink"
          >
            {product.name}
          </h1>
          ...
          <div
            data-reveal
            className="mt-4 flex items-center justify-between rounded-2xl border border-border-sand bg-sand-light px-5 py-4"
          >
            <div>
              ...
              <div className="mt-1 text-[11px] text-[#8a7c6c]">
                Paiement à la livraison disponible
              </div>
            </div>
          ...
          <div
            data-reveal
            className="mt-6 flex gap-2.5 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          >
            {TRUST_BADGES.map(({ icon: Icon, label }) => (
              ...
              >
                <Icon
                  className="h-3.5 w-3.5 shrink-0 text-brown"
                  strokeWidth={1.5}
                  aria-hidden="true"
                />
                <div className="text-[11px] leading-tight whitespace-nowrap text-ink">
                  {label}
                </div>
              </div>
            ))}
        ...
        <div className="border-t border-border-sand px-4 py-6">
          <details className="group py-4 first:pt-0" open>
            <summary className="flex cursor-pointer list-none items-center justify-between text-sm font-semibold text-ink">
              Fiche technique
              <ChevronDown
                className="h-4 w-4 text-brown transition-transform duration-200 group-open:rotate-180"
                aria-hidden="true"
              />
            </summary>
            <dl className="mt-3 flex flex-col gap-3 border-t border-border-sand pt-3">
              {SPECS.map(({ label, value }) => (
                ...
                >
                  ...
                  <dd className="text-right text-[13px] text-[#8a7c6c]">
                    {value(product)}
                  </dd>
                </div>
              ))}
